  ...
  try {
    ...
    const [orders, total] = await Promise.all([
      prisma.order.findMany({
        ...
        orderBy: { created_at: 'desc' },
        include: {
          product: { select: { id: true, name: true, price: true, seller_id: true } },
...
export const getMetrics = async (_req, res) => {
  try {
    const [productTotals, usersCount, ordersCount, revenue, topCategories] = await Promise.all([
      prisma.product.groupBy({ by: ['status'], _count: { _all: true } }),
      prisma.user.count(),
      prisma.order.count(),
      prisma.order.aggregate({ _sum: { }, }),
      prisma.product.groupBy({ by: ['category'], _count: { _all: true }, orderBy: { _count: { _all: 'desc' } }, take: 5 })
    ])
    res.json({
      success: true,
      data: {
